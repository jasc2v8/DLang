...
int doBuild(string sourceFile) {
	...
	if (appType == "GUI" ) {
		...
		auto pid = spawnProcess([ "cmd", "/k", 
		cmd,"--config=console", "&", "pause", "&", "exit"],
//		cmd,"--config=console", "&", "echo", "**** Type EXIT to return to GUI ****"],
		std.stdio.stdin, std.stdio.stdout);
		rtn = wait(pid);
	}
...
string doPipeShell(string cmd) {
	...
	foreach (line; pipes.stdout.byLine) {
	    //ignore non-errors, capture errors below
	}

	foreach (line; pipes.stderr.byLine) {
		//capture errors, if any
	    stderrBuffer ~= to!string(line);
	}
	//return errors if any, else ""
	return stderrBuffer;
}
